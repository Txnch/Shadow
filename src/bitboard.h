...
constexpr Bitboard BB_RANK_1 = 0xFFULL;
constexpr Bitboard BB_RANK_8 = BB_RANK_1 << 56;

constexpr Bitboard BB_FILE_MASK[8] = {
    BB_FILE_A,
    BB_FILE_A << 1,
    BB_FILE_A << 2,
    BB_FILE_A << 3,
...
constexpr Bitboard BB_RANK_MASK[8] = {
    BB_RANK_1,
    BB_RANK_1 << 8,
    BB_RANK_1 << 16,
    BB_RANK_1 << 24,
...
inline constexpr Bitboard north(Bitboard b) {
    return b << 8;
}

inline constexpr Bitboard south(Bitboard b) {
    return b >> 8;
}

inline constexpr Bitboard east(Bitboard b) {
    return (b & ~BB_FILE_H) << 1;
}

inline constexpr Bitboard west(Bitboard b) {
    return (b & ~BB_FILE_A) >> 1;
}

inline constexpr Bitboard north_east(Bitboard b) {
    return (b & ~BB_FILE_H) << 9;
}

inline constexpr Bitboard north_west(Bitboard b) {
    return (b & ~BB_FILE_A) << 7;
}

inline constexpr Bitboard south_east(Bitboard b) {
    return (b & ~BB_FILE_H) >> 7;
}

inline constexpr Bitboard south_west(Bitboard b) {
    return (b & ~BB_FILE_A) >> 9;
}
